// Existing canvas setup code remains unchanged
...
// Keyboard class with singleton pattern
const KeyboardModule = (function () {
  ...
  class Keyboard {
    ...
    areMovementKeysPressed() {
      return (
        this.ongoingActions.has(InputActions.MOVE_UP) ||
  ...
  return {
    getInstance: function () {
      if (!instance) {
        ...
        instance.bindKey("w", InputActions.MOVE_UP);
        instance.bindKey("s", InputActions.MOVE_DOWN);
        instance.bindKey("a", InputActions.MOVE_LEFT);
        instance.bindKey("d", InputActions.MOVE_RIGHT);
        instance.bindKey("f", InputActions.ACTION_1);
        instance.listen();
      }
      return instance;
    },
    destroy: function () {
      if (instance) {
        instance.unlisten();
        instance = null;
        console.log("Keyboard instance destroyed");
      }
    },
  };
})();
...
// Initialize keyboard
const k = KeyboardModule.getInstance();

// Game loop
function loop() {
  console.log("running loop");
  // Example: Check keyboard state
  if (k.areMovementKeysPressed()) {
    console.log("Movement keys pressed:", Array.from(k.ongoingActions));
  }
