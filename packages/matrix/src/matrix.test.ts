import { createFlatMatrix, createMatrix, getCoordinatesFromTile, getTileFromCoordinates, getTileRowAndColumn } from "./matrix";
...
describe("tile", () => {
  ...
  test("getCoordinatesFromTile", () => {
    ...
    expect(getCoordinatesFromTile(0, matrixConfig)).toEqual({ x: 0, y: 0 });
    expect(getCoordinatesFromTile(129, matrixConfig)).toEqual({ x: 144, y: 48 });
    expect(getCoordinatesFromTile(1199, matrixConfig)).toEqual({ x: 624, y: 464 });
  });
